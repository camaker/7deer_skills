...
上线: 2025-12-13
访问量: 170.8M visits
域名: survivezombiearena.net
临时 CF Pages: https://a1121481.survivezombiearena-net.pages.dev
```
...
| 竞品 | URL | 弱点 |
|------|-----|------|
| survivezombiearena.com (wiki) | survivezombiearena.com | 有首页但无结构化攻略 |
...
- 项目路径: `/Users/zirer/Projects/survivezombiearena-net/`
...
## 部署命令
```bash
cd /Users/zirer/Projects/survivezombiearena-net

# 首次创建 Cloudflare Pages 项目
npx wrangler pages project create survivezombiearena-net --production-branch=main

# 部署（每次更新）
npx wrangler pages deploy out/ --project-name=survivezombiearena-net

# 域名绑定（需要 Cloudflare Dashboard 手动操作）
# Pages → survivezombiearena-net → Custom domains → 添加 survivezombiearena.net
# CNAME 指向: a1121481.survivezombiearena-net.pages.dev
```
...
for page in codes tier-list classes weapons wave-guide loadouts credits-guide beginner-guide updates about terms privacy-policy; do
  echo -n "$page: "; curl -s -o /dev/null -w "%{http_code}" "https://a1121481.survivezombiearena-net.pages.dev/$page"
done

# sitemap 包含所有页面
curl -s https://a1121481.survivezombiearena-net.pages.dev/sitemap.xml | grep -o '<loc>[^<]*</loc>' | wc -l
# 应输出 26+
```
